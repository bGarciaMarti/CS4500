...
void * producer_thread( void *arg)
{
    ...
    struct list sillyList; // create a new linked list
    struct Node * ptr, tmp;
    int counter = 0;
    
    // initalize the new linked list
    sillyList.header = sillyList.tail = NULL;

    /* generate and attach K nodes to the global list */
    for (counter = 0; counter < K; counter++) {
        ptr = generate_data_node();
        if (List->header == NULL){ // if you're at the start of the linked list
            List->header = sillyList.header = ptr;
        } else {
            sillyList.tail->next = ptr;
            sillyList.tail = ptr;
        }
    }
    pthread_mutex_lock(&mutex); // retrieve the mutex lock to update the linked list
    if (List->header == NULL){ // if you're at the start of the linked list
        List->header = sillyList.header;
        List->tail = sillyList.tail;
    } else {
        List->tail->next = sillyList.header;
        List->tail = sillyList.tail;
    }
    pthread_mutex_unlock(&mutex); // return the mutex lock
}
